...
class FormHelper extends AppHelper
{
    ...
    protected $_options = array(
            'day' => array(), 'minute' => array(), 'hour' => array(), 'month' => array(), 'year' => array(),
            'meridian' => array()
    );
    /**
    ...
    protected function _getModel($model)
    {
        ...
        if(ClassRegistry::isKeySet($model)){
            ...
            $object = ClassRegistry::init(array(
                    'class' => $plugin.$this->request->params['models'][$model]['className'], 'alias' => $model
            ));
        }elseif(ClassRegistry::isKeySet($this->defaultModel)){
            $defaultObject = ClassRegistry::getObject($this->defaultModel);
            if($defaultObject && in_array($model, array_keys($defaultObject->getAssociated()), true)
               && isset($defaultObject->{$model})
            ){
        ...
        $this->fieldset[$model] = array(
                'fields' => null, 'key' => $object->primaryKey, 'validates' => null
        );
        return $object;
    }
     ...
     *                         - `fieldset` Set to false to disable the fieldset. If a string is supplied it will be
     *                         used as the classname for the fieldset element.
     *                         - `legend` Set to false to disable the legend for the generated input set. Or supply a
    ...
    public function inputs($fields = null, $blacklist = null, $options = array())
    {
        ...
        if($legend === true){
            ...
            $isEdit     = (strpos($this->request->params['action'], 'update') !== false
        ...
        foreach($fields as $name => $options){
            ...
            $entity      = explode('.', $name);
            $blacklisted = (is_array($blacklist)
                            && (in_array($name, $blacklist)
                                || in_array(end($entity), $blacklist)));
            if($blacklisted){
                continue;
            }
    ...
    public function input($fieldName, $options = array())
    {
        ...
        $type   = $options['type'];
        $out    = array(
                'before' => $options['before'], 'label' => $label, 'between' => $options['between'],
                'after' => $options['after']
        );
    ...
    protected function _magicOptions($options)
    {
        ...
        if(isset($options['options'])){
            ...
            $map        = array(
                    'string' => 'text', 'datetime' => 'datetime', 'boolean' => 'checkbox', 'timestamp' => 'datetime',
                    'text' => 'textarea', 'time' => 'time', 'date' => 'date', 'float' => 'number', 'integer' => 'number'
            );
    ...
    {
        $defaults = array(
                'wrap' => true, 'class' => 'error-message', 'escape' => true
        );
    ...
    public function hidden($fieldName, $options = array())
    {
        ...
        $options = $this->_initInputField($fieldName, array_merge($options, array('secure' => self::SECURE_SKIP)));
    ...
    public function checkbox($fieldName, $options = array())
    {
        ...
        if($options['hiddenField']){
            $hiddenOptions = array(
                    ...
                    'value' => ($options['hiddenField'] !== true ? $options['hiddenField'] : '0'), 'secure' => false
            );
    ...
    public function select($fieldName, $options = array(), $attributes = array())
    {
        ...
        $tag    = null;
        $attributes += array(
                'class' => null, 'escape' => true, 'secure' => true, 'empty' => '', 'showParents' => false,
                'hiddenField' => true, 'disabled' => false
        );
        ...
        if(!empty($attributes['multiple'])){
            ...
            if($hiddenField){
                $hiddenAttributes = array(
                        'value' => '', 'id' => $attributes['id'].($style ? '' : '_'), 'secure' => false,
        ...
        if(!empty($tag) || isset($template)){
            ...
            if((!isset($secure) || $secure) && empty($attributes['disabled'])
        ...
        $emptyMulti = ($showEmpty !== null && $showEmpty !== false
        ...
        $select   = array_merge($select, $this->_selectOptions(array_reverse($options, true), array(), $showParents, array(
                'escape' => $escapeOptions, 'style' => $style, 'name' => $attributes['name'],
                'value' => $attributes['value'], 'class' => $attributes['class'], 'id' => $attributes['id'],
                'disabled' => $attributes['disabled'],
        )));
        ...
        foreach($elements as $name => $title){
            ...
            if($name !== null){
                ...
                if($showParents || (!in_array($title, $parents))){
                    ...
                    if($hasDisabled && !$disabledIsArray){
                        $htmlOptions['disabled'] = $attributes['disabled']
                                                   === true ? 'disabled' : $attributes['disabled'];
                    }
                    if($attributes['style'] === 'checkbox'){
    ...
    public function dateTime($fieldName, $dateFormat = 'DMY', $timeFormat = '12', $attributes = array())
    {
        ...
        $defaults   = array(
                'minYear' => null, 'maxYear' => null, 'separator' => '-', 'interval' => 1, 'monthNames' => true
        );
        ...
        if($hasId && is_array($attributes['id'])){
            ...
            $attributes['id'] += array(
                    'month' => '', 'year' => '', 'day' => '', 'hour' => '', 'minute' => '', 'meridian' => ''
            );
            foreach($keys as $key){
        ...
        if(is_array($attributes['empty'])){
            $attributes['empty'] += array(
                    'month' => true, 'year' => true, 'day' => true, 'hour' => true, 'minute' => true, 'meridian' => true
            );
            foreach($keys as $key){
        ...
        switch($timeFormat){
            case '24':
                ...
                $opt .= $this->hour($fieldName, true, $attrs['Hour']).'時'. //時に修正
            ...
            case '12':
                ...
                $opt .= $this->hour($fieldName, false, $attrs['Hour']).'時'. //時に修正
                        $this->minute($fieldName, $attrs['Minute']).' '.$this->meridian($fieldName, $attrs['Meridian']);
                break;
        }
        return $opt;
    }
    ...
    protected function _getDateTimeValue($value, $timeFormat)
    {
        ...
        if(is_array($value)){
            ...
            return array(
                    $year, $month, $day, $hour, $min, $meridian
            );
        }
        if(is_numeric($value)){
    ...
    public function year($fieldName, $minYear = null, $maxYear = null, $attributes = array())
    {
        ...
        $yearOptions = array(
                'value' => $attributes['value'], 'min' => $minYear, 'max' => $maxYear, 'order' => 'desc'
        );
    ...
    public function month($fieldName, $attributes = array())
    {
        ...
        return $this->select($fieldName
                             .".month", $this->_generateOptions('month', array('monthNames' => $monthNames)), $attributes);
    }
    ...
    public function hour($fieldName, $format24Hours = false, $attributes = array())
    {
        ...
        return $this->select($fieldName
                             .".hour", $this->_generateOptions($format24Hours ? 'hour24' : 'hour'), $attributes);
    }
    ...
    public function create($model = null, $options = array())
    {
        ...
        if($model !== false && $key){
            $recordExists = (isset($this->request->data[$model]) && !empty($this->request->data[$model][$key])
        ...
        $options = array_merge(array(
                'type' => ($created && empty($options['action'])) ? 'put' : 'post', 'action' => null, 'url' => null,
                'default' => true, 'encoding' => strtolower(Configure::read('App.encoding')), 'inputDefaults' => array()
        ), $options);
        ...
        if($options['action'] === null && $options['url'] === null){
            ...
            $actionDefaults    = array(
                    'plugin' => $plugin, 'controller' => $this->_View->viewPath, 'action' => $options['action'],
            );
    ...
    {
        $options += array(
                'type' => 'submit', 'escape' => false, 'secure' => false
        );
        if($options['escape']){
            $title = h($title);
        }
     ...
     * array('label' => 'save', 'name' => 'Whatever', 'div' => array('class' => 'good')); <div class="good">
    ...
    public function submit($caption = null, $options = array())
    {
        ...
        $options += array(
                'type' => 'submit', 'before' => null, 'after' => null, 'secure' => false
        );
    ...
    public function secure($fields = array())
    {
        ...
        $out      = $this->hidden('_Token.fields', array(
                'value' => urlencode($fields.':'.$locked), 'id' => 'TokenFields'.mt_rand()
        ));
        $out .= $this->hidden('_Token.unlocked', array(
                'value' => urlencode($unlocked), 'id' => 'TokenUnlocked'.mt_rand()
        ));
    ...
    public function postLink($title, $url = null, $options = array(), $confirmMessage = false)
    {
        ...
        $formOptions = array(
                'name' => $formName, 'id' => $formName, 'style' => 'display:none;', 'method' => 'post',
        );
        ...
        $out .= $this->Html->useTag('hidden', '_method', array('value' => $requestMethod));
        $out .= $this->_csrfField();
        $fields = array();
        if(isset($options['data']) && is_array($options['data'])){
            foreach($options['data'] as $key => $value){
