<div class="content_view" style="margin:0 auto; max-width:500px; word-break:break-all;">
    <?php echo nl2br($this->Project->sanitize($content['ReportContent']['txt_content'])) ?>
</div>
...
<div class="content_edit_form clearfix" style="display: none;">
    ...
    <div class="form-group text_content_form">
        <?php echo $this->Form->input('txt_content', array(
                ...
                'value' => $content['ReportContent']['txt_content']
        )); ?>
    </div>

    <div class="form-group" id="movie_btns">
        <button type="button"
                onclick="edit_text($(this), '<?php echo $this->Html->url(array(
                        'controller' => 'report_contents', 'action' => 'text_edit'
                )) ?>');"
                class="btn btn-primary col-xs-3" style="margin-right:20px;">更新
        </button>
        <button type="button" onclick="cancel_edit($(this));" class="btn btn-primary col-sm-3"
                style="margin-right:20px;">キャンセル
        </button>
    </div>
