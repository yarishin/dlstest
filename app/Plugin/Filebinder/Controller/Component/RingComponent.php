...
class RingComponent extends Component
{
    ...
    private function _checkBindUpped($array)
    {
        ...
        $keys = array(
                'field_name', 'file_name', 'file_content_type', 'file_size', 'tmp_bind_path'
        );
    ...
    private function _bindUp(Model $model, &$data, $i = null)
    {
        ...
        foreach($data as $fieldName => $value){
            ...
            if(!$this->_checkFileUploaded($value) || $value['error'] == UPLOAD_ERR_NO_FILE
               || !empty($this->controller->request->data[$model->alias]['delete_'.$fieldName])
            ){
                $data[$fieldName] = null;
                continue;
            }
            ...
            $tmpBindPath = $tmpPath.'ring_'.date('YmdHis').'_'.Security::hash($model->alias.$fieldName.$fileName.time())
                           .$fileName;
            // move_uploaded_file
            move_uploaded_file($tmpFile, $tmpBindPath);
            $ring             = array(
                    'model' => $model->alias, 'field_name' => $fieldName, 'file_name' => $fileName,
                    'file_content_type' => $contentType, 'file_size' => $fileSize, 'tmp_bind_path' => $tmpBindPath
            );
            $data[$fieldName] = $ring;
        }
        $sessionKey = is_int($i) ? "Filebinder.{$model->alias}.{$i}" : "Filebinder.{$model->alias}";
