...
class BindableBehavior extends ModelBehavior
{
    ...
    public function setUp(Model $model, $settings = array())
    {
        ...
        $defaultRuntime = array(
                'bindedModel' => null, 'primaryKey' => 'id',
        );
        // Default settings
    ...
    protected function bindFile(Model $model, $data = array())
    {
        ...
        $query           = array();
        $query['fields'] = array(
                'id', 'model', 'model_id', 'field_name', 'file_name', 'file_content_type', 'file_size', 'created',
                'modified'
        );
        // with Object
        ...
        $query['conditions'] = array(
                'model' => $modelName, 'model_id' => $model_ids
        );
        ...
        if($dataSource->config['datasource'] === 'Mongodb.MongodbSource'){
            $query['conditions'] = array(
                    'model' => $modelName, 'model_id' => array('$in' => $model_ids)
            );
        }
        $binds = $this->runtime[$model->alias]['bindedModel']->find('all', $query);
        $binds = Set::combine($binds, array(
                '%1$s.%2$s', '/'.$this->settings[$model->alias]['model'].'/model_id',
                '/'.$this->settings[$model->alias]['model'].'/field_name'
        ), '/'.$this->settings[$model->alias]['model']);
                ...
                if(array_key_exists($model_id.'.'.$fieldName, $binds)){
                    ...
                    if((!file_exists($filePath) || filemtime($filePath) < strtotime($bind['modified']))){
                        ...
                        if($filePath && $dbStorage){
                            ...
                            if(!file_put_contents($filePath, base64_decode($fileObject))
                        ...
                        if($filePath && $s3Storage){
                            if(!class_exists('AmazonS3') || !Configure::read('Filebinder.S3.key')
                               || !Configure::read('Filebinder.S3.secret')
                            ){
                            ...
                            $responce = $s3->get_object($bucket, $urlPrefix
                                                                 .$model->transferTo(array_diff_key($bind, Set::normalize(array('file_object')))), array('fileDownload' => $filePath,));
                            if(!$responce->isOK()){
                     ...
                     */
                    if($this->settings[$model->alias]['withObject']
                       && empty($tmpData[$key][$modelName][$fieldName]['file_object'])
                    ){
                        /**
                         * S3 storage
                         */
                        ...
                        if($s3Storage){
                            ...
                            $responce    = $s3->get_object($bucket, $urlPrefix
                                                                    .$model->transferTo(array_diff_key($bind, Set::normalize(array('file_object')))), array('fileDownload' => $tmpFilePath,));
                            if(!$responce->isOK()){
                                //__('Validation Error: S3 Upload Error');
                                return false;
                            }
    ...
    public function beforeSave(Model $model, $options = array())
    {
        ...
        foreach($model->data[$modelName] as $fieldName => $value){
            ...
            unset($model->data[$modelName][$fieldName]);
            $model->bindedData[$modelName][$fieldName] = $data[$this->runtime[$model->alias]['bindedModel']->alias]
                                                         + array($this->runtime[$model->alias]['bindedModel']->primaryKey => $bind_id);
        }
        return true;
    }
    ...
    public function afterSave(Model $model, $created, $options = array())
    {
        ...
        foreach($model->bindedData[$modelName] as $fieldName => $value){
            ...
            if($delete || (!$created && !empty($value['tmp_bind_path']))){
                ...
                if($delete || $this->settings[$model->alias]['exchangeFile']){
                    ...
                    $this->runtime[$model->alias]['bindedModel']->deleteAll(array(
                            'model' => $modelName, 'model_id' => $model_id, 'field_name' => $fieldName
                    ));
                }
            }
            ...
            $baseDir = empty($bindFields[$fieldName]['filePath']) ? $this->settings[$model->alias]['filePath'] : $bindFields[$fieldName]['filePath'];
            if($baseDir){
                $filePath = $baseDir.$model->transferTo(array_diff_key(array('model_id' => $model_id)
                                                                       + $value, Set::normalize(array('tmp_bind_path'))));
            }else{
                $filePath = false;
            }
            $tmpFile = $value['tmp_bind_path'];
            if(file_exists($tmpFile) && is_file($tmpFile)){
                ...
                if($s3Storage){
                    ...
                    $urlPrefix = !empty($bindFields[$fieldName]['urlPrefix']) ? $bindFields[$fieldName]['urlPrefix'] : Configure::read('Filebinder.S3.urlPrefix');
                    $responce  = $s3->create_object($bucket, $urlPrefix
                                                             .$model->transferTo(array_diff_key(array('model_id' => $model_id)
                                                                                                + $value, Set::normalize(array('tmp_bind_path')))), array(
                            'fileUpload' => $tmpFile, 'acl' => $acl,
                    ));
                    if(!$responce->isOK()){
                        //__('Validation Error: S3 Upload Error');
                        @unlink($tmpFile);
    ...
    public function deleteEntity(Model $model, $modelId, $fields = array())
    {
        ...
        $result = $this->runtime[$model->alias]['bindedModel']->deleteAll(array(
                'model' => $model->alias, 'model_id' => $modelId, 'field_name' => array_keys($deleteFields)
        ));
        if($result){
            $bindFields = Set::combine($model->bindFields, '/field', '/');
            foreach($bindFields as $fieldName => $value){
                ...
                if($s3Storage){
                    ...
                    $urlPrefix = !empty($bindFields[$fieldName]['urlPrefix']) ? $bindFields[$fieldName]['urlPrefix'] : Configure::read('Filebinder.S3.urlPrefix');
                    $responce  = $s3->delete_object($bucket, $urlPrefix.$model->transferTo($deleteFields[$fieldName]));
                    if(!$responce->isOK()){
                ...
                //					$result = false;
                //				}
                //上記の変わりに下記を実行して対象ディレクトリ内のファイルを全部削除するようにする
                //サムネイル作成をしている為(Logicky修正)
                if(!$filePath){
                    return false;
                }
                $pathinfo = pathinfo($filePath);
                $flag     = true;
                foreach(glob($pathinfo['dirname'].'/*') as $val){
                    if(!@unlink($val)){
                        $flag = false;
                    }
                }
    ...
    protected function _findBindedFields(Model $model, $modelId, $fields = array())
    {
        $query = array(
                'conditions' => array(
                        'model' => $model->alias, 'model_id' => $modelId,
                ), 'fields' => array(
                        'id', 'model', 'model_id', 'field_name', 'file_name', 'file_content_type', 'file_size',
                        'created', 'modified'
                ), 'recursive' => -1
        );
        if($fields){
        ...
        if($data){
            $data = Set::combine($data, '{n}.'.$this->runtime[$model->alias]['bindedModel']->alias.'.field_name', '{n}.'
                                                                                                                  .$this->runtime[$model->alias]['bindedModel']->alias);
        }
        return $data;
    }
    ...
    /**
     * Checkまずいコード　Logicky追加
     * Validation method: check file size
     * @param Model $model
     * @param       $value
     * @return bool
     */
    public function checkIllegalCode(Model $model, $value)
    {
        $filePath = $this->get_file_path($model, $value);
        $fp       = fopen($filePath, "r");
    ...
    /**
     * checkIllegalCode用のfile_path取得関数
     */
    private function get_file_path($model, $value)
    {
        $file = array_shift($value);
        if(!is_array($file)){
            return false;
     ...
     * @return bool
     */
    public function resize_image(Model $model, $tmp_file_path, $w, $h)
    {
        return $this->_resize_image($model, $tmp_file_path, $tmp_file_path, $w, $h);
    }

    /**
     * 画像リサイズ
     */
    private function _resize_image(Model $model, $file_path, $out_path, $nw = null, $nh = null)
    {
        list($ow, $oh, $type) = getimagesize($file_path);
        if(empty($ow) || empty($oh)){
            return false;
        }
        $x = 0;
        $y = 0;
        ...
        imagealphablending($canvas, false); //ブレンドモードを無効にする
        imagesavealpha($canvas, true); //完全なアルファチャネル情報を保存するフラグをonにする
        //画像インスタンス生成
        switch($type){
            case 1: //gif
        ...
        imagecopyresampled($canvas,  // 背景画像
                ...
                0,        // 背景画像のはりつけ位置（x座標）
                0,        // 背景画像のはりつけ位置（y座標）
                $x,        // コピー元画像のコピー開始位置（x座標）
                $y,        // コピー元画像のコピー開始位置（y座標）
                $nw,   // 背景画像のはりつける幅
                $nh,  // 背景画像のはりつける高さ
                $ow, // コピー元画像のコピーする幅
                $oh  // コピー元画像のコピーする高さ
        ...
        switch($type){
            case 1: //gif
                imagegif($canvas, $out_path);
                break;
            case 2: //jpeg
                imagejpeg($canvas, $out_path, 100);
                break;
            case 3: //png
                imagepng($canvas, $out_path, 9);
        }
        imagedestroy($canvas);
        return true;
    }
    ...
    /**
     * サムネイルの作成（外部URLの画像を内部に保存する）
     */
    public function create_thumbnail_from_out($model, $url, $save_path, $args)
    {
        if(!$args){
            return true;
        }
        foreach($args as $arg){
            ...
            $out_path = $pathinfo['dirname'].DS.$pathinfo['filename'].'_'.$arg[0].'x'.$arg[1].'.'
                        .$pathinfo['extension'];
            $this->_resize_image($model, $url, $out_path, $arg[0], $arg[1]);
        }
        return true;
    }
    ...
    {
        $args = array(array(48, 48)); //サムネイルのサイズ
        $save_dir_path = DS.'upload'.DS.'User'.DS.$user_id.DS.'img';
        $dir = new Folder(WWW_ROOT.$save_dir_path);
        if($dir->find('.*')) return true;
        $dir->create(WWW_ROOT.$save_dir_path);
        $url = str_replace('_normal', "_400x400", $twitter_img_url); //_normalを400に置換
        $url_info = parse_url($url);
        $path_info = pathinfo($url_info['path']);
        $file_name = substr(str_shuffle('1234567890abcdefghijklmnopqrstuvwxyz'), 0, 10);
        $save_path = WWW_ROOT.$save_dir_path.DS.$file_name.'.'.$path_info['extension'];
        if($this->resize_image_from_out($model, $url, $save_path, 400, 400)){ //100*100の画像
     ...
     * 外の画像をリサイズしたうえで保存する
     */
    public function save_image_from_out($model, $url, $id, $field, $sizes)
    {
        $save_dir_path = DS.'upload'.DS.$model->name.DS.$id.DS.$field;
        new Folder(WWW_ROOT.$save_dir_path, true, 0757);
        $this->_delete_files_in_dir(WWW_ROOT.$save_dir_path);
        $url_info = parse_url($url);
        $path_info = pathinfo($url_info['path']);
        $file_name = substr(str_shuffle('1234567890abcdefghijklmnopqrstuvwxyz'), 0, 10);
        $save_path = WWW_ROOT.$save_dir_path.DS.$file_name.'.'.$path_info['extension'];
        $size = array_shift($sizes);
        if($this->resize_image_from_out($model, $url, $save_path, $size[0], $size[1])){
            if(!$this->create_thumbnail($model, $save_path, $sizes)){
                return false;
            }
            $Attachment = $this->_getModelObject('Attachment', 'attachments');
            $Attachment->deleteAll(array('model' => $model->name, 'model_id' => $id, 'field_name' => $field), false);
            $data = array(
                'Attachment' => array(
                    'model' => $model->name,
                    'model_id' => $id,
    ...
    private function _delete_files_in_dir($dir_path)
    {
        foreach(glob($dir_path.DS.'*') as $val){
            @unlink($val);
        }
    }
     ...
     * @return bool
     */
    public function resize_image_from_out($model, $url, $save_path, $w, $h)
    {
        return $this->_resize_image($model, $url, $save_path, $w, $h);
    }
     ...
     * @param array  $args サムネイルサイズの配列 array(array(w, h), array(w, h...))
     * @return boolean
     */
    public function create_thumbnail(Model $model, $file_path, $args)
    {
        if(!$args) return true;
        foreach($args as $arg){
            $pathinfo = pathinfo($file_path);
            $out_path = $pathinfo['dirname'].'/'.$pathinfo['filename'].'_'.$arg[0].'x'.$arg[1].'.'
                        .$pathinfo['extension'];
            $this->_resize_image($model, $file_path, $out_path, $arg[0], $arg[1]);
        }
        return true;
    }
    ...
    public function _getModelObject($className, $table = null)
    {
        ...
        $object = new Model(array(
                'name' => $className, 'table' => $table, 'ds' => null
        ));
