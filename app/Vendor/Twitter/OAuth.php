...
class OAuthSignatureMethod_HMAC_SHA1 extends OAuthSignatureMethod
{
    ...
    public function build_signature($request, $consumer, $token)
    {
        ...
        $key_parts            = array(
                $consumer->secret, ($token) ? $token->secret : ""
        );
...
class OAuthRequest
{
    ...
    public static function from_consumer_and_token($consumer, $token, $http_method, $http_url, $parameters = null)
    {
        @$parameters or $parameters = array();
        $defaults = array(
                "oauth_version" => OAuthRequest::$version, "oauth_nonce" => OAuthRequest::generate_nonce(),
                "oauth_timestamp" => OAuthRequest::generate_timestamp(), "oauth_consumer_key" => $consumer->key
        );
        if($token){
...
class OAuthServer
{
    ...
    private function get_signature_method(&$request)
    {
        ...
        if(!in_array($signature_method, array_keys($this->signature_methods))){
            throw new OAuthException("Signature method '$signature_method' not supported "."try one of the following: "
